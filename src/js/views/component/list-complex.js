...
var ComplexListItem = React.createClass({
	...
	render: function () {
		...
		return (
			<Link to="details" viewTransition="show-from-right"
				params={{
					user: this.props.user,
					prevView: 'component-complex-list',
				}} className="list-item" component="div">
				<UI.ItemMedia avatar={this.props.user.img}
					avatarInitials={initials} />
				<div className="item-inner">
					<div className="item-content">
						<div className="item-title">{[
							this.props.user.name.first,
							this.props.user.name.last].join(' ')}</div>
						<div className="item-subtitle">
							{this.props.user.location}
						</div>
					</div>
					<UI.ItemNote type="default"
						label={this.props.user.joinedDate.slice(-4)}
		...
		return (
			<UI.View>
				<UI.Headerbar type="default" label="Complex List">
					<UI.HeaderbarButton showView="home"
						viewTransition="reveal-from-right" label="Back"
